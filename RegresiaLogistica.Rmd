...
Realizam modelul pentru variabilele relevante.
```{r}
...
#model_final <- glm(data=train, potability~aluminium+barium+chloramine+chromium+copper, family=binomial)
summary(model_final)
```
...
```{r}
pred_mx <- predict(model_final, newdata=test, target="class")
pred_mx
(conf <- confusionMatrix(pred_mx, factor(test$potability)))
```
